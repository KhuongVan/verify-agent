...
import { saveAlbum, type Album, type Item, type ItemBytes, type MediaKind } from '@/lib/store';
...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    items.push({ id, kind, mimeType, ext: extFromMime(mimeType), sizeBytes: bytes.length, sha256, signatureB64, keyId });
